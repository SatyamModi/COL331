...
uint64_t fibb_util(uint64_t n, addr_t *pmain_stack, addr_t *pf_stack) 
{
  if(n<=2) return 1;
  else 
  {
    ...
    addr_t &f_stack = *pf_stack;
    stack_saverestore(f_stack, main_stack);
    return fibb_util(n-1, pmain_stack, pf_stack)+fibb_util(n-2, pmain_stack, pf_stack);
  }
}

void fibb(addr_t *pmain_stack, addr_t *pf_stack, uint64_t *pret, bool *pdone, uint64_t n)
{
  addr_t &main_stack = *pmain_stack;
  addr_t &f_stack = *pf_stack;
  uint64_t &ret = *pret;
  bool &done = *pdone;
  ret = fibb_util(n, pmain_stack, pf_stack);
...
void shell_step_fiber_scheduler(shellstate_t& shellstate,addr_t& main_stack, preempt_t& preempt, addr_t stackptrs[], size_t stackptrs_size, addr_t arrays, size_t arrays_size, dev_lapic_t& lapic){
  ...
  if (shellstate.taskResources[taskIndex].cmd && (b1 || b2))
  {
    if (shellstate.taskResources[taskIndex].init == true)
    {
      ...
      // stack_saverestore(main_stack, stackptrs[taskIndex]);
    }
  }
